...
const FermBatch = () => {
  ...
  const handleBatchChange = async () => {
    ...
    try {
      ...
      console.log('reponse for batch', response);
      // setBatchList(response.map(i => ({ value: i.batch })));
      setBatchList(
  ...
  const getFermentationData = async () => {
    const { batchNumber } = getTopFiledsData();
    console.log('batch number in getFermentationData', batchNumber);
    try {
    ...
    setFormData(prevState => {
      const updatedTableData = prevState.tableData.map(formItem => {
        ...
        if (serverItem) {
          ...
          console.log('Updated row:', mergedItem); // Debugging log
          return mergedItem;
        }
        return formItem;
      });
  ...
  const handleSubmit = async formData => {
    try {
      ...
      console.log('batchInsertQuery', batchInsertQuery);
  ...
  const generateFermentationDataInsertQuery = (tableData, topFields) => {
    ...
    const values = tableData.map(row => {
      console.log('selected time of start', row.time_of_start);
      // console.log("selected date",typeof row.time_of_start)
      console.log('row time data', moment(row.time_of_start).format('HH:mm'));
        ...
        {Object.keys(formData.topFields).map(key => (
          ...
          >
            ...
            {key === 'dateOfPropStart' ? (
              ...
              //   onChange={e =>
              //     handleFieldChange('topFields', key, e.target.value)
              //   }
              //   style={{
              //     border: 'none',
              //     borderBottom: '1px solid #000',
              ...
              <CustomDropdown
                itemList={batchList}
                createNewItem={createBatchListItem}
                setItemList={setBatchList}
